...
export class Projects extends Component {
  ...
  render() {
    return (
      ...
      <ListView background="#f1f2f4" width="240" height="500px">
        <ListViewHeader paddingBottom="5px">
        ...
        <ListViewSection header={this.renderSectionHeader('Projects')}>
          {this.state.projectInfo.map((prop, key) => {
            if (key%2 === 0) {
              if (prop == "snakeGame") {
                return this.renderItemWithLink(key, prop)
              } else {
        ...
        <ListViewFooter>
          <Link href='https://github.com/stanliest' target="_blank" rel="noopener" size="13">github.com/stanliest</Link>
        </ListViewFooter>
        </ListView>
      </Window>
  ...
  renderItem(title, info) {
    return (
      ...
      >
        <svg x="0px" y="0px" width="18" height="12" viewBox="0 0 18 12" style={{ marginRight: '7px' }}>
    ...
    17,6.7 "/>
        ...
        <Text color="#414141" size="14">{info}</Text>
      </ListViewRow>
    );
  }

  renderItemWithLink(title, info) {
    return (
      <ListViewRow
        onClick={() => this.setState({ selected: title })}
        background={this.state.selected === title ? '#d8dadc' : null}
    ...
    17,6.7 "/>
        ...
        <Link color="blue" size="14"><a target="_blank" rel="noopener" href="snakeGame/index.html">{info}</a></Link>
      </ListViewRow>
    );
  }
  ...
  renderDescription(title, info) {
    return (
      <ListViewRow
        ...
        background={this.state.selected === title ? '#d8dadc' : null}
      >
        <Text size="13" color="#696969">{info}</Text>
      </ListViewRow>
    );
  }
}
